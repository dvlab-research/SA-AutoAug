...
class AnchorGenerator(nn.Module):
    ...
    def __init__(
        ...
        if len(anchor_strides) == 1:
            ...
            cell_anchors = [
                generate_anchors(anchor_stride, sizes, aspect_ratios).float(), atss_on
            ]
        else:
...
def make_anchor_generator(config):
    ...
    anchor_generator = AnchorGenerator(
        anchor_sizes, aspect_ratios, anchor_stride, straddle_thresh, atss_on
    )
...
def generate_anchors(
    stride=16, sizes=(32, 64, 128, 256, 512), aspect_ratios=(0.5, 1, 2), atss_on=False,
):
...
def _generate_anchors(base_size, scales, aspect_ratios, atss_on=False):
    """Generate anchor (reference) windows by enumerating aspect ratios X
    scales wrt a reference (0, 0, base_size - 1, base_size - 1) window.
    """
    _sub = 0.5 if atss_on else 1
    anchor = np.array([1, 1, base_size, base_size], dtype=np.float) - _sub
