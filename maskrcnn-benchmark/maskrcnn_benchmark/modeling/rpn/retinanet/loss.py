...
class RetinaNetLossComputation(RPNLossComputation):
    ...
    def _count_loss_scale(self, matched_targets_boxes, pos_inds, box_loss_vec, cls_loss_vec):
        ...
        loss_vec = cls_loss_vec.sum(dim=1) + box_loss_vec.sum(dim=1)
        loss_scale = [loss_vec.detach(), gt_areas.detach()]
        return loss_scale

    def __call__(self, anchors, box_cls, box_regression, targets, search=False):
        """
        Arguments:
        ...
        if search:
            loss_scale = self._count_loss_scale(matched_targets_boxes, pos_inds, box_loss_vec/(max(1, pos_inds.numel() * self.regress_norm)), cls_loss_vec[pos_inds]/(pos_inds.numel() + N))
        else:
            loss_scale = None
